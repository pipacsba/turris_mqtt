...
//Defines for MQTT
#define ADDRESS     "tcp://xy.xy.xy.xy:1883"
...
int main (int argc, char *argv[])
{
	...
	while(!done)
	{
		...
		if (MQTT_Connected >= 1)
		{
			snprintf(mqtt_payload,100,"{\"CPU_temp\": %.3f, \"load_1\": %.2f, \"mqtt\": %i, \"load_5\": %.2f, \"load_15\": %.2f, \"uptime\": %li}",
				systemp, load_1, MQTT_Connected, load_5, load_15, info.uptime);
